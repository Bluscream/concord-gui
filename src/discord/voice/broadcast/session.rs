...
impl BroadcastVideoTransport {
    ...
    pub async fn send_encrypted_rtp(&self, socket: &UdpSocket, packet: &[u8]) -> Result<usize, String> {
        let encrypted = self.packet_encryptor.encrypt_media_packet(packet)?;
        socket
            .send(&encrypted)
            .await
...
pub fn packet_pacing_interval(packet_count: usize, rate_limited_budget: Duration) -> Option<Duration> {
