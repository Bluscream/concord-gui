...
async fn main() -> Result<()> {
    ...
    let app = App::new();
    // The cache is a front end's decision, not the core's: this crate depends
    // on it, the core does not, and a build that wants none simply passes
    // `None` here.
    app.run_with(concord_cache::open_from_config().await).await
}
...
fn check_config() -> Result<()> {
    ...
    crate::tui::validate_keymap_options(&keymap)?;
    ...
    for warning in app_warnings
        ...
        .chain(crate::tui::theme_options_warnings(&theme))
    {
