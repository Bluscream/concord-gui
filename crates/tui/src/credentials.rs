...
pub(super) async fn resolve_token(auth_session: DiscordAuthSession) -> Result<ResolvedToken> {
    ...
    if let Some(token) = token_store::env_token() {
        // The fake takes the same door as a real account, so running against
        // it is `CONCORD_TOKEN=demo` and nothing else. Checked before
        // validation because it is not a token and would rightly be refused.
        #[cfg(feature = "fixtures")]
        if crate::demo::is_demo_token(&token) {
            return Ok(ResolvedToken {
                token,
                warnings: vec![
                    "running against the built-in fake, not a Discord account".to_owned(),
                ],
            });
        }
