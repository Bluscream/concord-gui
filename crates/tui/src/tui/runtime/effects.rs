...
fn missing_members_for_effect(
    ...
    let messages = match event {
        ...
        | AppEvent::MessageSearchLoaded {
            page: concord::discord::MessageSearchPage { messages, .. },
        }
...
fn dispatch_desktop_notification(notification: DesktopNotification, icon: Option<String>) {
    ...
    if let Some(sequence) =
        concord::support::macos_notification::sequence_for_current_terminal(&title, &body)
    {
...
pub(in crate::tui) fn dispatch_push_to_talk_sound(pressed: bool) {
    tokio::spawn(async move {
        let result =
            tokio::task::spawn_blocking(move || concord::sound::play_push_to_talk_sound(pressed))
                .await;
        match result {
            Ok(Ok(())) => {}
            Ok(Err(error)) => {
...
fn play_voice_sound(
    ...
    #[cfg(feature = "voice-playback")]
    {
        concord::sound::play_voice_sound(kind, custom_path)
    }
...
fn play_notification_sound(
    ...
    #[cfg(feature = "voice-playback")]
    {
        concord::sound::play_notification_sound(custom_path)
    }
    #[cfg(not(feature = "voice-playback"))]
    {
        let _ = custom_path;
...
mod tests {
    ...
    fn effect_waits_until_snapshot_revision_catches_up() {
        ...
        let mut media_runtime =
            DashboardMediaRuntime::new(concord::config::ImageProtocolPreference::Auto);
    ...
    fn push_guild_with_channel(
        ...
        guild_id: Id<concord::discord::ids::marker::GuildMarker>,
        channel: ChannelInfo,
    ) {
        state.push_event(guild_create_event(GuildCreateFixture {
    ...
    fn channel_info(
        guild_id: Id<concord::discord::ids::marker::GuildMarker>,
        channel_id: Id<concord::discord::ids::marker::ChannelMarker>,
        parent_id: Option<Id<concord::discord::ids::marker::ChannelMarker>>,
        name: &str,
        kind: &str,
    ) -> ChannelInfo {
        ChannelInfo {
    ...
    fn message_info(
        guild_id: Id<concord::discord::ids::marker::GuildMarker>,
        channel_id: Id<concord::discord::ids::marker::ChannelMarker>,
        message_id: Id<concord::discord::ids::marker::MessageMarker>,
        author_id: Id<concord::discord::ids::marker::UserMarker>,
    ) -> MessageInfo {
        MessageInfo {
            guild_id: Some(guild_id),
            author_id,
