...
impl DashboardCommandScheduler {
    ...
    async fn schedule_forum_post_data(
        ...
        if let Some(concord::discord::ForumPostDataRequestTarget {
            guild_id,
            channel_id,
            thread_ids,
    ...
    async fn schedule_archived_threads(
        ...
        if let Some(concord::discord::ArchivedThreadRequestTarget {
            guild_id,
            channel_id,
            cursor,
...
mod tests {
    use super::*;
    use concord_fixtures::events::{GuildCreateFixture, guild_create_event};
