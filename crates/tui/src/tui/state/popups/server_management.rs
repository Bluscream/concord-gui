...
use concord::discord::{AppCommand, AuditLogEntryInfo, GuildEmojiInfo, GuildInviteInfo};
...
pub(in crate::tui) struct ServerManagementState {
    ...
    pub(super) roles: Vec<concord::discord::RoleState>,
    ...
    pub(super) sounds: Vec<concord::discord::SoundboardSound>,
    pub(super) automod: Vec<concord::discord::AutoModRule>,
    pub(super) audit_log: Vec<AuditLogEntryInfo>,
    pub(super) welcome: Option<concord::discord::WelcomeScreen>,
    pub(super) widget: Option<concord::discord::GuildWidget>,
    ...
    pub(super) events: Vec<concord::discord::ScheduledEvent>,
    pub(super) templates: Vec<concord::discord::GuildTemplate>,
    pub(super) member_rows: Vec<concord::discord::MemberRow>,
    /// What is being searched for, while the field is open.
    pub(super) member_query: Option<TextInputState>,
    pub(super) onboarding: Option<concord::discord::Onboarding>,
    pub(super) onboarding_rows: Vec<concord::discord::OnboardingRow>,
    pub(super) onboarding_picked: Vec<u64>,
    pub(super) discovery: Option<concord::discord::DiscoveryMetadata>,
    pub(super) discovery_categories: Vec<concord::discord::DiscoveryCategory>,
    pub(super) stickers: Vec<concord::discord::GuildSticker>,
...
impl ServerManagementState {
    ...
    pub(in crate::tui) fn sounds(&self) -> &[concord::discord::SoundboardSound] {
        &self.sounds
    }

    pub(in crate::tui) fn automod(&self) -> &[concord::discord::AutoModRule] {
        &self.automod
    }
    ...
    pub(in crate::tui) fn roles(&self) -> &[concord::discord::RoleState] {
        &self.roles
    }
...
impl DashboardState {
    ...
    pub(in crate::tui) fn guild_stickers(&self) -> &[concord::discord::GuildSticker] {
        self.popups
            .server_management()
    ...
    pub(in crate::tui) fn set_guild_stickers(
        &mut self,
        stickers: Vec<concord::discord::GuildSticker>,
    ) {
    ...
    pub(in crate::tui) fn set_discovery_metadata(
        &mut self,
        metadata: concord::discord::DiscoveryMetadata,
        categories: Vec<concord::discord::DiscoveryCategory>,
    ) {
        if let Some(state) = self.popups.server_management_mut() {
    ...
    pub(in crate::tui) fn set_onboarding(&mut self, onboarding: concord::discord::Onboarding) {
        if let Some(state) = self.popups.server_management_mut() {
    ...
    pub(in crate::tui) fn onboarding_rows(&self) -> &[concord::discord::OnboardingRow] {
        self.popups
            .server_management()
    ...
    pub(in crate::tui) fn member_rows(&self) -> &[concord::discord::MemberRow] {
        self.popups
            .server_management()
    ...
    pub fn submit_emoji_rename(&mut self) -> Option<AppCommand> {
        ...
        let index = match edit {
            ...
            EmojiEdit::GuildName => {
                if !concord::discord::is_valid_guild_name(&text) {
                    return None;
                }
            ...
            EmojiEdit::EditEvent(event_id) => {
                let event = concord::discord::parse_new_event(&text)?;
            ...
            EmojiEdit::AddImage => {
                ...
                let Some(name) = concord::discord::emoji_name_from_filename(&text) else {
                    self.show_error_toast(
    ...
    pub(in crate::tui) fn set_scheduled_events(
        &mut self,
        events: Vec<concord::discord::ScheduledEvent>,
    ) {
        if let Some(state) = self.popups.server_management_mut() {
            state.events = events;
            state.loading = false;
    ...
    pub(in crate::tui) fn set_guild_templates(
        &mut self,
        templates: Vec<concord::discord::GuildTemplate>,
    ) {
        if let Some(state) = self.popups.server_management_mut() {
    ...
    pub(in crate::tui) fn scheduled_events(&self) -> &[concord::discord::ScheduledEvent] {
        self.popups
            .server_management()
            .map_or(&[], |state| state.events.as_slice())
    }

    pub(in crate::tui) fn guild_templates(&self) -> &[concord::discord::GuildTemplate] {
        self.popups
            .server_management()
    ...
    pub(in crate::tui) fn set_welcome_screen(&mut self, screen: concord::discord::WelcomeScreen) {
        if let Some(state) = self.popups.server_management_mut() {
    ...
    pub(in crate::tui) fn set_guild_widget(&mut self, widget: concord::discord::GuildWidget) {
        if let Some(state) = self.popups.server_management_mut() {
            ...
            ServerPanelTab::Membership => {
                ...
                match row {
                    ...
                    MembershipRow::PruneDays => {
                        ...
                        state.prune_days = concord::discord::PRUNE_DAYS[next];
    ...
    fn can_edit_role(&self, guild_id: Id<GuildMarker>, role: &concord::discord::RoleState) -> bool {
    ...
    fn channel_id_by_name(
        ...
        guild_id: Id<GuildMarker>,
        name: &str,
    ) -> Option<Id<concord::discord::ids::marker::ChannelMarker>> {
    ...
    fn cycle_guild_verification(&mut self, guild_id: Id<GuildMarker>) -> Option<AppCommand> {
        use concord::discord::GuildVerificationLevel as Level;
    ...
    fn fill_guild_settings(&mut self) {
        ...
        let settings = vec![
            ...
            (
                ...
                concord::discord::verification_label(guild.verification_level.unwrap_or_default()),
            ),
            (
                "Owner".to_owned(),
                guild
    ...
    pub(in crate::tui) fn apply_panel_sounds(
        ...
        sounds: Vec<concord::discord::SoundboardSound>,
    ) {
