...
use concord::discord::{
    ...
    RichPresenceSelection, RoleState, UserProfileInfo, UserProfileUpdate,
};
...
impl DashboardState {
    ...
    pub fn activate_selected_member_action(&mut self) -> Option<AppCommand> {
        ...
        match item.kind {
            ...
            | MemberActionKind::Unblock => {
                ...
                self.request_risky(concord::risk::RiskKind::FriendAction, command)
            }
            MemberActionKind::Kick
            | MemberActionKind::Ban
            | MemberActionKind::Timeout
    ...
    pub fn save_user_profile_settings_command(&mut self) -> Option<AppCommand> {
        ...
        // Upstream's accessor, our risk path: editing a profile through a
        // third-party client is one of the watched actions, so it is explained
        // before it is sent.
        popup.set_settings_status(Some("Saving profile changes...".to_owned()));
        self.request_risky(
            concord::risk::RiskKind::ProfileEdit,
