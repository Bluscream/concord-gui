...
use concord::discord::{AppCommand, DiscordAction, custom_emoji_image_url};
...
impl super::super::DashboardState {
    ...
    pub fn thread_edit_view(&self) -> Option<ThreadEditView> {
        ...
        let parent_channel = channel
            .parent_id
            .and_then(|parent_id| self.discord.cache.channel(parent_id));
        let available_tags: &[concord::discord::ForumTagInfo] =
    ...
    ) -> Vec<concord::discord::ForumTagInfo> {
        self.discord
            .cache
            .channel(thread_id)
...
fn forum_tag_order(
    available_tags: &[concord::discord::ForumTagInfo],
...
impl DashboardState {
    pub(in crate::tui::state) fn forum_tag_emoji_fields(
        &self,
        tag: &concord::discord::ForumTagInfo,
        guild_id: Option<Id<concord::discord::ids::marker::GuildMarker>>,
    ) -> ForumTagEmojiFields {
