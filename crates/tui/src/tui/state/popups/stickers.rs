...
use concord::discord::StickerInfo;
use concord::discord::ids::{Id, marker::StickerMarker};
...
impl DashboardState {
    ...
    pub fn stage_selected_sticker(&mut self) {
        ...
        if self.composer.pending_stickers.len() >= concord::discord::MAX_MESSAGE_STICKERS {
            return;
        }
