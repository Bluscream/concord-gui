...
use concord::discord::{AppCommand, ReactionEmoji};
...
impl DashboardState {
    ...
    pub fn existing_emoji_reactions(&self) -> &[concord::discord::ReactionEmoji] {
        self.popups
            .emoji_reaction_picker()
    ...
    pub fn own_emoji_reactions(&self) -> &[concord::discord::ReactionEmoji] {
        self.popups
            .emoji_reaction_picker()
...
fn prioritize_existing_reactions(
    items: Vec<EmojiReactionItem>,
    existing_reactions: &[concord::discord::ReactionEmoji],
) -> Vec<EmojiReactionItem> {
...
fn emoji_reaction_is_remaining_unicode(item: &EmojiReactionItem) -> bool {
    matches!(&item.emoji, concord::discord::ReactionEmoji::Unicode(emoji) if !is_quick_unicode_emoji(emoji))
}
