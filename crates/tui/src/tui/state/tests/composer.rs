...
fn empty_guild_channel_locks_until_a_message_exists() {
    ...
    state.push_event(AppEvent::MessageHistoryLoadFailed {
        channel_id: Id::new(2),
        target: concord::discord::MessageHistoryLoadTarget::Latest,
...
fn long_message_file_send_rechecks_attachment_permission_and_capacity() {
    ...
    full.add_pending_composer_attachments(
        (0..concord::discord::MAX_UPLOAD_ATTACHMENT_COUNT)
            .map(|index| {
...
fn pending_attachments_are_capped_at_upload_limit() {
    ...
    state.confirm_selected_channel();
    state.start_composer();
    let attachments = (0..concord::discord::MAX_UPLOAD_ATTACHMENT_COUNT + 2)
        .map(|index| {
    ...
    assert_eq!(
        ...
        concord::discord::MAX_UPLOAD_ATTACHMENT_COUNT
    );
}
