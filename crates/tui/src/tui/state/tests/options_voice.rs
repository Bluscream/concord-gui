...
fn joined_voice_channel_can_select_a_stream_target_and_stop_sharing() {
    ...
    assert_eq!(
        ...
        Some(AppCommand::LoadStreamCaptureTargets {
            request_id: concord::discord::StreamCaptureTargetsRequestId::new(0),
            scope: VoiceScope::Guild(guild_id),
            channel_id,
        })
    );
    ...
    assert_eq!(
        ...
        Some(AppCommand::LoadStreamCaptureTargets {
            request_id: concord::discord::StreamCaptureTargetsRequestId::new(1),
            scope: VoiceScope::Guild(guild_id),
            channel_id,
        })
    );
...
mod privacy {
    use super::*;
    use concord::discord::{DmScanLevel, FriendSources, PrivacyEdit};
    ...
    fn toggling_one_friend_source_carries_the_others() {
        ...
        state.push_event(AppEvent::UserSettingsUpdate {
            settings: concord::discord::UserSettingsInfo {
                friend_source_flags: Some(concord::discord::UserFriendSourceFlagsInfo {
                    all: None,
...
mod access {
    use super::*;
    use concord::discord::{AuthSession, AuthorisedApp};
