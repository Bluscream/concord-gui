...
fn channel_action_menu_toggle_mute_opens_duration_then_dispatches_command() {
    ...
    assert_eq!(
        ...
        Some(AppCommand::SetChannelMuted {
            ...
            muted: true,
            duration: Some(concord::discord::MuteDuration::Minutes(15)),
...
fn the_server_panel_only_fetches_a_tab_it_has_not_seen() {
    ...
    state.apply_guild_invites(
        guild_id,
        vec![concord::discord::GuildInviteInfo {
            code: "aBc-123".to_owned(),
            channel_id: None,
            channel_name: None,
            inviter: None,
    ...
    state.set_welcome_screen(concord::discord::WelcomeScreen::default());
    state.set_guild_widget(concord::discord::GuildWidget::default());
    ...
    state.set_onboarding(concord::discord::Onboarding::default());

    assert_eq!(
        state.next_server_tab(),
        Some(AppCommand::LoadDiscoveryMetadata { guild_id })
    );
    state.set_discovery_metadata(concord::discord::DiscoveryMetadata::default(), Vec::new());
...
fn the_audit_log_offers_no_row_action() {
    ...
    state.apply_guild_audit_log(
        guild_id,
        vec![concord::discord::AuditLogEntryInfo {
            id: Id::new(7001),
...
fn renaming_an_emoji_seeds_the_field_and_applies_locally() {
    ...
    state.apply_guild_emojis(
        guild_id,
        vec![concord::discord::GuildEmojiInfo {
            id: Id::new(8001),
...
fn only_the_guilds_own_sounds_are_manageable() {
    ...
    state.apply_panel_sounds(
        None,
        vec![concord::discord::SoundboardSound {
            sound_id: 1,
...
fn blocking_one_guilds_direct_messages_keeps_the_other_restrictions() {
    ...
    state.push_event(AppEvent::UserSettingsUpdate {
        settings: concord::discord::UserSettingsInfo {
...
mod membership {
    ...
    fn a_prune_with_members_to_remove_is_offered_for_the_chosen_window() {
        ...
        assert!(concord::discord::PRUNE_DAYS.contains(&days));
    }
...
mod event_line {
    use concord::discord::parse_new_event;
    use concord::discord::{NewEventLocation, NewEventProblem};
