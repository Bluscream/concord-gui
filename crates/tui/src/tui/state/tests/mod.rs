...
use concord::discord::{
    ...
    ChannelUnreadState, CustomEmojiInfo, DiscordState, DownloadAttachmentSource, EmbedFieldInfo,
    EmbedInfo, ForumTagInfo, GuildFolder, GuildMemberListItem, GuildMemberListOperation,
    GuildMemberListUpdateInfo, GuildNotificationSettingsInfo, MESSAGE_FLAG_IS_COMPONENTS_V2,
...
fn state_with_voice_channel_participant() -> DashboardState {
    ...
    state.push_event(concord_fixtures::events::guild_create_event(
        GuildCreateFixture {
            channels: vec![
