...
use concord::AppError;
use concord::discord::ActionBlockReason;
...
use concord::discord::{AppCommand, ReplyReference};
...
impl DashboardState {
    ...
    pub fn can_send_in_selected_channel(&self) -> bool {
        match self.selected_channel_state() {
            ...
            Some(channel) => self
                .discord
                .cache
                .channel_action_decision(channel, concord::discord::DiscordAction::SendMessage)
    ...
    fn submit_ready_composer_command(&mut self, command: AppCommand) -> Option<AppCommand> {
        ...
        match &command {
            AppCommand::SendFriendRequest { .. } => {
                return self.request_risky(concord::risk::RiskKind::FriendAction, command);
            }
            AppCommand::UpdateUserProfile { .. } => {
                return self.request_risky(concord::risk::RiskKind::ProfileEdit, command);
            }
            _ => {}
        }
    ...
    fn finish_composer_message_submission(
        ...
        self.composer.pending_composer_attachment_previews.clear();
        let nonce = concord::discord::next_message_nonce();
        self.stage_pending_message(channel_id, nonce, &content, reply_to, &attachments);
        AppCommand::SendMessage {
            channel_id,
            nonce,
    ...
    ) -> concord::Result<MessageSendLimits> {
        let limits =
        ...
        let channel =
            self.discord
                ...
                .ok_or(AppError::DiscordActionBlocked {
                    ...
                    reason: concord::discord::ActionBlockReason::ChannelDataUnavailable,
                })?;
    ...
    fn selected_composer_guild_id(&self) -> Option<Id<concord::discord::ids::marker::GuildMarker>> {
        self.selected_channel_state()
            .and_then(|channel| channel.guild_id)
            .or_else(|| self.selected_guild_id())
    }

    fn composer_role_candidates(&self) -> Vec<&concord::discord::RoleState> {
        self.selected_composer_guild_id()
            .map(|guild_id| self.discord.cache.roles_for_guild(guild_id))
            .unwrap_or_default()
    }

    fn composer_everyone_role_id(&self) -> Option<Id<concord::discord::ids::marker::RoleMarker>> {
        self.selected_composer_guild_id()
            .map(|guild_id| Id::new(guild_id.get()))
    }

    fn composer_channel_candidates(&self) -> Vec<&concord::discord::ChannelState> {
        self.discord
            .cache
    ...
    fn command_option_value_candidates(
        &self,
        value_query: &str,
        option: &concord::discord::ApplicationCommandOptionInfo,
    ) -> Vec<CommandPickerEntry> {
            ...
            BuiltinSlashCommandParse::Ready(BuiltinSlashCommandSubmit::Message {
                ...
                if tts {
                    ...
                    BuiltinCommandSubmit::Ready(AppCommand::SendTtsMessage {
                        channel_id,
                        nonce: concord::discord::next_message_nonce(),
                        content,
                    })
                } else {
