    ...
    match key.code {
        ...
        KeyCode::Char('P') => {
            if let Some(command) = state.pending_prune() {
                return state.request_risky(concord::risk::RiskKind::PruneMembers, command);
            }
            return None;
        }
        // Deleting, which enter deliberately does not do on any of these tabs.
        KeyCode::Char('d') => {
            return state
                .delete_selected_automod_rule()
                .or_else(|| state.remove_selected_event())
                .or_else(|| state.delete_selected_template());
        }
    ...
    if state.is_bulk_ban_open() {
        match state.key_bindings().composer_action(key) {
            ComposerAction::Submit => {
                ...
                return state.request_risky(concord::risk::RiskKind::BulkBan, command);
            }
