...
use concord::discord::ComponentMediaItemInfo;
...
fn message_embed_groups_documented_inline_fields() {
    ...
    message.embeds = vec![EmbedInfo {
        fields: vec![
            concord::discord::EmbedFieldInfo {
...
fn message_content_highlights_role_mentions_with_role_name() {
    ...
    let mut options = concord::config::ThemeOptions::default();
    options
...
fn thread_created_message_shows_archived_and_locked_after_the_title() {
    ...
    state.push_event(AppEvent::ChannelUpsert(ChannelInfo {
        ...
        message_count: Some(12),
        total_message_sent: Some(14),
        thread_metadata: Some(concord::discord::ThreadMetadataInfo::test(true, true)),
        ..ChannelInfo::test(Id::new(10), "thread")
    }));
...
fn translated_message_is_rendered_below_the_original_with_branch_anchor() {
    ...
    let (request_id, message_id) = match command {
        concord::discord::AppCommand::Translate {
            request_id,
            target: concord::discord::TranslationTarget::Message(message_id),
            ..
