...
fn post_form_state(
    ...
    DashboardState,
    Id<concord::discord::ids::marker::ChannelMarker>,
    Id<concord::discord::ids::marker::ChannelMarker>,
) {
...
use crate::tui::state::{
    ActionAvailability, ConfirmationButton, MessageConfirmationKind, ReactionUsersPopupState,
};
use crate::tui::ui::{downloads_popup_area, downloads_popup_lines};
use concord::discord::FriendStatus;
...
fn owned_stream_info_omits_local_capture_source_without_viewers() {
    ...
    let scope = concord::discord::VoiceScope::Guild(Id::new(1));
...
fn stream_info_panel_lists_owned_and_multiple_watched_streams() {
    ...
    state.push_event(AppEvent::StreamCreate {
        stream: concord::discord::StreamCreateInfo {
...
fn popup_lists_use_the_configured_selection_marker() {
    let (options, parse_warnings) =
        concord::config::parse_theme_options_for_test("[ui.indicator]\nselection = \"❯ \"\n")
...
fn popup_key_sequence_hint_renders_above_the_active_modal() {
    let mappings = [
        (
            "JumpTop".to_owned(),
            concord::config::KeymapBinding::one("z z"),
        ),
        (
            "StartComposer".to_owned(),
            concord::config::KeymapBinding::one("z s"),
        ),
    ]
    .into_iter()
    .collect();
...
fn leader_key_sequence_hint_shows_keymap_entries_alongside_defaults() {
    ...
    mappings.insert(
        "StartComposer".to_owned(),
        concord::config::KeymapBinding::one("<leader>e"),
    );
    mappings.insert(
        "ReplyMessage".to_owned(),
        concord::config::KeymapBinding::one("<leader>m r"),
    );
    mappings.insert(
        "ChannelSwitcher".to_owned(),
        concord::config::KeymapBinding::one("<leader><C-w>"),
    );
    let mut state = DashboardState::new_with_options(
        Default::default(),
        Default::default(),
...
fn leader_action_popup_renders_modified_action_shortcut_labels() {
    ...
    channel_actions.insert(
        "ToggleMute".to_owned(),
        concord::config::KeymapBinding::one("<C-u>"),
    );
    let mut state = DashboardState::new_with_options(
        Default::default(),
        Default::default(),
