...
fn reaction_emoji_cell(
    emoji: &concord::discord::ReactionEmoji,
    show_custom_emoji: bool,
    ready_urls: &[String],
) -> String {
...
fn reaction_user_lines(
    ...
    max_visible_lines: usize,
    inner_width: usize,
    display_name: impl Fn(&concord::discord::ReactionUserInfo) -> String,
) -> Vec<Line<'static>> {
...
fn reaction_emoji_label(
    emoji: &concord::discord::ReactionEmoji,
    show_custom_emoji: bool,
) -> String {
    match emoji {
        concord::discord::ReactionEmoji::Custom { id, .. } if !show_custom_emoji => {
            id.get().to_string()
        }
...
pub(in crate::tui::ui) fn emoji_reaction_picker_lines_with_own_reactions(
    reactions: &[EmojiReactionItem],
    own_reactions: &[concord::discord::ReactionEmoji],
    selected: usize,
    max_visible_items: usize,
    thumbnail_urls: &[String],
) -> Vec<Line<'static>> {
...
struct EmojiReactionPickerRenderOptions<'a> {
    ...
    own_reactions: &'a [concord::discord::ReactionEmoji],
    show_custom_emoji: bool,
    filter: Option<&'a str>,
    max_width: usize,
}
...
fn format_emoji_reaction_item(
    ...
    match &reaction.emoji {
        concord::discord::ReactionEmoji::Unicode(emoji) => format!("{} {}", emoji, reaction.label),
        concord::discord::ReactionEmoji::Custom { id, .. } if !show_custom_emoji => {
            format!("{} {}", id.get(), reaction.label)
        }
        concord::discord::ReactionEmoji::Custom { .. } if thumbnail_ready => format!(
            "{}{}",
            " ".repeat(usize::from(EMOJI_REACTION_IMAGE_WIDTH.saturating_add(1))),
            reaction.label
        ),
        concord::discord::ReactionEmoji::Custom { name, .. } => name
            .as_deref()
