//! A fake Discord that answers commands.
//!
//! The other half of a fake world: [`crate::world`] builds the state, and this
//! answers what a front end does to it. Sending a message appends it and shows
//! somebody typing a reply a moment later; reacting updates the count; loading
//! history hands back another page. Enough to click through a client and see
//! it behave, with no account and no network.
//!
//! It answers in events rather than by writing state directly, because that is
//! how the real gateway answers. A front end driven by this exercises the same
//! path it uses against Discord, which is the only reason a fake is worth
//! having - one that took a shortcut would prove the shortcut works.
//!
//! Deliberately free of any front end's plumbing: it takes a command and
...
pub enum Emission {
    /// An event the gateway would have sent.
    Event(Box<AppEvent>, DiscordState),
    /// State moved with no event to show for it - an optimistic local edit,
    /// say. Front ends that only redraw on events would otherwise miss it.
    State(DiscordState),
}
...
impl FakeBackend {
    ...
    /// The account this fake is signed in as.
    pub fn user_id(&self) -> concord::discord::Id<concord::discord::marker::UserMarker> {
        crate::world::demo_user_id()
    }
    ...
    /// When the next deferred action is due, if any.
    ///
    /// Returned rather than slept on here: a front end already has an event
    /// loop, and a fake that blocked in its own thread would be a second one
    /// to reason about.
    ...
    pub fn fire_due(&mut self) -> Vec<Emission> {
        let mut out = Vec::new();
        fire_due(&mut self.state, &mut out, &mut self.pending);
        out
    }
}
...
fn fire_due(
    ...
    *pending = rest;
    ...
    out.push(Emission::State(state.clone()));
}
...
fn handle_command(
    ...
    // Each emission carries the state as it stood when it happened, rather
    // than one snapshot at the end: two events in a single command are two
    // different states, and collapsing them would show the front end the
    // second one twice.
    macro_rules! publish_event {
        ($event:expr) => {
            out.push(Emission::Event(Box::new($event), state.clone()))
        };
    }
