...
impl Dialect {
    ...
    pub fn guarded_upsert(self, table: &str, columns: &[&str]) -> String {
        ...
        match self {
            ...
            //
            // `deleted = 0` rides along: a write that outranks a tombstone is
            // the row coming back, and without clearing the flag a recreated
            // channel or a rejoined guild would stay hidden for good. It is
            // inside the same guarded statement, so a write that loses to the
            // tombstone does not resurrect anything.
            Self::Sqlite => {
                format!("{base}, deleted = 0 WHERE excluded.revision >= {table}.revision")
            }
            ...
            Self::MySql => {
                ...
                format!(
                    "INSERT INTO {table} ({names}) VALUES ({placeholders}) \
                     ...
                     deleted = IF(VALUES(revision) >= revision, 0, deleted)"
                )
            }
        }
    }
    ...
    pub fn schema(self) -> Vec<String> {
        ...
        vec![
            // Records which build last shaped this store. Keyed by a name
            // rather than a bare row so a later value can be added without
            // another table.
            format!(
                "CREATE TABLE IF NOT EXISTS schema_meta (
                     name {short} NOT NULL PRIMARY KEY,
                     value {int} NOT NULL
                 )"
            ),
            format!(
            ...
            format!(
                // Stickers are small and fully described by what Discord
                // sends, so unlike an embed there is nothing left over that a
                // replay would have to leave out.
            ...
            format!(
                // The metadata, not the bytes. A cached URL still needs the
                // network to draw, but it makes the message replayable: the
                // layout is right and the picture fills in, rather than the
                // whole message being withheld.
            ...
            format!(
                // Keyed by guild and user together, spelled as one synthetic
                // id rather than a composite primary key: every other table
                // here is keyed by a single `id` column, and the tombstone and
                // upsert helpers are written against that shape.
                "CREATE TABLE IF NOT EXISTS members (
                    id {short} NOT NULL PRIMARY KEY,
                    guild_id {id},
                    user_id {id},
                    display_name {short},
                    nickname {short},
                    ...
                    is_bot {int} NOT NULL DEFAULT 0,
                    joined_at {short},
                    -- Comma-separated. A join table would be the tidy answer,
                    -- but nothing here queries by role, and a second table
                    -- would double the writes for a list that is read whole.
                    role_ids {text},
                    revision {int} NOT NULL DEFAULT 0,
                    deleted {int} NOT NULL DEFAULT 0
                 )"
            ...
            format!(
                "CREATE TABLE IF NOT EXISTS messages (
                    ...
                    -- Whether the message had attachments, embeds, stickers or
                    -- a poll. None of those are cached, so a message that had
                    -- one is not replayed: rendering it without its picture is
                    -- wrong in the way that looks like a bug, where a message
                    -- that is simply absent looks like loading.
                    has_extras {int} NOT NULL DEFAULT 0,
                    updated_at {int} NOT NULL DEFAULT 0,
                    -- Discord's own ordering stamp, so two clients resolve a
                    -- conflict the same way without agreeing about the time.
                    revision {int} NOT NULL DEFAULT 0,
