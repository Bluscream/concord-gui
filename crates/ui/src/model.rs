//! The vocabulary both front ends use to describe what is on screen.
//!
//! Panes, action-menu entries, and whether an action is available. Named for
//! the terminal client because that is where they started, but none of it is a
//! terminal idea: both front ends have a guild list and a member list, and
//! both have to decide whether to offer an action before offering it.
//!
//! Here rather than in either front end so the key bindings that resolve to
//! these can be shared instead of written twice and drifting.
...
impl ThreadActionKind {
    pub const fn discord_action(self) -> Option<DiscordAction> {
        match self {
...
impl ThreadNotificationItem {
    pub fn new(raw_label: &str, flags: u64, current_flags: u64) -> Self {
...
impl ChannelPaneEntry<'_> {
    ...
    pub fn cursor(&self) -> ChannelPaneCursor {
        match self {
            Self::CategoryHeader { state, .. }
            | Self::Channel { state, .. }
...
/// Which half of the notification inbox is showing.
///
/// Here rather than beside the terminal inbox because a key binding resolves
/// to switching it, and the bindings are shared.
