...
use super::Theme;
use concord::config::{HighlightGroup, ThemeOptions};
...
pub fn resolved_highlights(
    ...
    HighlightGroup::ALL
        ...
        .copied()
        .map(|group| {
            // Converted rather than used directly: the terminal build hands
            // back ratatui's types, and this half of the crate always works in
            // its own. Without the feature both sides are already the same
            // type and the conversion is the identity.
            let style = crate::style::Style::from(theme.style(group));
            ...
            (
                ...
                ResolvedStyle {
                    ...
                    strikethrough: modifiers.contains(Modifier::CROSSED_OUT),
                },
            )
        })
        .collect()
